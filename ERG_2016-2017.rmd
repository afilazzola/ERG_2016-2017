...
data <- read.table("Weather.2016.csv", header=T,sep=",")
...
HOBO.data <- read.csv("HOBOdata2016.csv")
...
```
### Biomass differences among phytometers

```{r echo=FALSE, fig.width=14, fig.height=7}
biomass <- read.csv("Biomass2016.csv")
biomass[is.na(biomass)] <- 0
biomass[,"total.phyto"] <- rowSums(biomass[,6:8])
biomass2 <- subset(biomass, Other>0) ## drop zeros for ambient

bio.avg <- biomass %>% group_by(Microsite, Gradient) %>% summarize(phacelia=mean(Phacelia),plantago=mean(Plantago),salvia=mean(Salvia),other.spp=mean(Other),total.phyto=mean(total.phyto))
bio.avg <- data.frame(bio.avg)

bio.se <- biomass %>% group_by(Microsite, Gradient) %>% summarize(phacelia=se(Phacelia),plantago=se(Plantago),salvia=se(Salvia),other.spp=se(Other),total.phyto=se(total.phyto))
bio.se <- data.frame(bio.se)

par(mfrow=c(1,2))
plot(xax-0.1, bio.avg[bio.avg$Microsite=="shrub","other.spp"], pch=19, ylim=c(0,22), xlab="", xaxt="n", ylab="ambient biomass (g)", cex.axis=1.3, cex.lab=1.5, cex=1.5, xlim=c(0.7,7.3))
error.bar(xax-0.1, bio.avg[bio.avg$Microsite=="shrub","other.spp"],bio.se[bio.se$Microsite=="shrub","other.spp"]*1.96,lwd=2)
error.bar(xax+0.1,  bio.avg[bio.avg$Microsite=="open","other.spp"], bio.se[bio.se$Microsite=="open","other.spp"]*1.96, lwd=2)
points(xax+0.1,  bio.avg[bio.avg$Microsite=="open","other.spp"], pch=21, bg="White", cex=1.5)
#legend(5.8,20, c("shrub","open"), pch=21, pt.bg=c("black","white"), bty="n", cex=2)


## plot each species
species.plot <- function(spp) {
plot(xax-0.1, bio.avg[bio.avg$Microsite=="shrub",spp], pch=19, ylim=c(0,3), xlab="", xaxt="n", ylab="phytometer biomass (g)", cex.axis=1.3, cex.lab=1.5, cex=1.5, xlim=c(0.7,7.3))
error.bar(xax-0.1, bio.avg[bio.avg$Microsite=="shrub",spp],bio.se[bio.se$Microsite=="shrub",spp]*1.96,lwd=2)
error.bar(xax+0.1,  bio.avg[bio.avg$Microsite=="open",spp], bio.se[bio.se$Microsite=="open",spp]*1.96, lwd=2)
points(xax+0.1,  bio.avg[bio.avg$Microsite=="open",spp], pch=21, bg="White", cex=1.5)
legend(5,3, c("shrub","open"), pch=21, pt.bg=c("black","white"), bty="n", cex=1.5)
}

species.plot("total.phyto")

## Biomass for ambient community
m1 <- aov(log(Other)~Site*Microsite, data=biomass2)
summary(m1)
# shapiro.test(m1$residuals) #normal after log transform

##Biomass for all the phytometers
biomass3 <- subset(biomass, total.phyto>0) ## drop zeros for phyto
m2 <- aov(log(total.phyto)~Site*Microsite*Nutrients, data=biomass3)
summary(m2)
# shapiro.test(m2$residuals) #normal after log transform

# ##Phacelia
# bio.phac <- subset(biomass, Phacelia>0)
# m3 <- aov(log(Phacelia)~Site*Microsite*Nutrients, data=bio.phac)
# shapiro.test(m3$residuals)
# 
# 
# ##Salvia
# bio.sal <- subset(biomass, Salvia>0)
# m4 <- aov(log(Salvia)~Site*Microsite*Nutrients, data=bio.sal)
# shapiro.test(m4$residuals)
```

### Interactions along gradient

Relative interaction index for ambient biomass and all phytometers. Positive values represent facilitation while negative values represent an adverse shrub effect. Error bars are 95% confidence intervals. 
...
rii.bio <- rii(biomass, c("Site","Gradient","Microsite"),c("Phacelia","Other","total.phyto"))

rii.avg <- rii.bio %>% group_by(Gradient) %>% summarize(other=mean(Other),Total.phyto=mean(total.phyto), other.se=se(Other), phyto.se=se(total.phyto), phacelia=mean(Phacelia), phac.se=se(Phacelia))
rii.avg <- data.frame(rii.avg)


par(mfrow=c(1,2))

par(mar=c(4.5,4.5,.5,.5))
plot(rii.avg[,"Gradient"],rii.avg[,"other"], xaxt="n", ylab="Rii Biomass", xlab="", pch=19, cex=1.5, ylim=c(-0.3,0.7), xlim=c(0.7,7.3))
error.bar(rii.avg[,"Gradient"],rii.avg[,"other"],rii.avg[,"other.se"]*1.96)
abline(h=0, lty=2, lwd=2)
axis(1, xax, unique(site), cex.axis=1.3)
text(6.5,0.68, "ambient", cex=2)


par(mar=c(4.5,3.5,.5,.5))
plot(rii.avg[,"Gradient"],rii.avg[,"Total.phyto"], xaxt="n", ylab="", xlab="", pch=19, cex=1.5, ylim=c(-0.3,0.7), xlim=c(0.7,7.3))
error.bar(rii.avg[,"Gradient"],rii.avg[,"Total.phyto"],rii.avg[,"phyto.se"]*1.96)
abline(h=0, lty=2, lwd=2)
axis(1, xax, unique(site), cex.axis=1.2)
text(6.5,0.68, "phytometer", cex=2)
