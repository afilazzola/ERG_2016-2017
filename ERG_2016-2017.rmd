...
DeMartonne's aridity gradient for each of the seven sites
...
```{r soil analyses, echo=FALSE, warning=FALSE, message=FALSE, fig.width=18, fig.height=7}
...
rii.bio <- rii(biomass, c("Site","Gradient","Microsite"),c("Other","total.phyto"))

rii.avg <- rii.bio %>% group_by(Gradient) %>% summarize(other=mean(Other),Total.phyto=mean(total.phyto), other.se=se(Other), phyto.se=se(total.phyto))
