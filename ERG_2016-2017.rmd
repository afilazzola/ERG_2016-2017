...
### Microenvironmnetal differences
...
bio.avg <- biomass %>% group_by(Microsite, Gradient) %>% summarize(phacelia=mean(Phacelia),plantago=mean(Plantago),salvia=mean(Salvia),other.spp=mean(Other))
bio.avg <- data.frame(bio.avg)

plot(xax-0.1, bio.avg[bio.avg$Microsite=="shrub","other.spp"], pch=19, ylim=c(0,22), xlab="", xaxt="n", ylab="biomass (g)", cex.axis=1.3, cex.lab=1.5, cex=1.5, xlim=c(0.7,7.3))
