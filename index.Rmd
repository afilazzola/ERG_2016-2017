...
summary(m1)
m1 <- lm(ihs(native) ~ poly(arid.gradient,2), data=mean.spp)
summary(m1)
...
comm <- community %>% group_by(Site, Microsite) %>% summarise_if(is.numeric, funs(sum(., na.rm=T)))
comm <- comm[,c(1:2,13:53)] ## extract species abundances only
comm <- data.frame(comm) ## convert to dataframe
comm[,"site.micro"] <- paste(comm$Site,comm$Microsite) ## create site by microsite column
rownames(comm) <- comm[,length(comm)] ## replace row names with site by microsite
comm[,c("Site","Microsite","Erinoginum.spp","Boraginaceae.sp.","site.micro")] <- NULL ## drop all columns but ones for analysis
...
## format dataframe
mpd.data <- data.frame(Microsite=c("open","shrub"),site=rownames(comm), mpd=mpd.data)
mpd.data[,"Site"] <- gsub(" open", "", mpd.data[,"site"], ) 
mpd.data[,"Site"] <- gsub(" shrub", "", mpd.data[,"Site"], ) 
mpd.data[,"site"] <- NULL

## getspecies richness for sites
spp.rich <- community %>% group_by(Site, Microsite) %>% summarize(richness=mean(Richness))
spp.rich <- data.frame(spp.rich)

## join data
mpd.rich <- merge(mpd.data, spp.rich, by=c("Site","Microsite"))

mpd.arid <- merge(mpd.rich, mean.spp, by=c("Site","Microsite"))
