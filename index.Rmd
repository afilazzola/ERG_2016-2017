...
```{r}
arid <- read.csv("Data//aridityCRU.csv")

arid <- data.frame(Site=rep(arid$name,2),gradient=rep(arid$gradient,2), year=c(rep(2016,7),rep(2017,7)),aridity=c(arid$arid2016,arid$arid2017))
...
## phosphorus
m3 <- lm(phosphorus ~ poly(aridity,2)* microsite, data=nutrient.arid)
summary(m3)
anova(m3)
r.squaredGLMM(m2) ## R squared values 
