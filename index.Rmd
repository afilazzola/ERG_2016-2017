...
mean.spp <- subset(mean.spp, Year==2016)
mean.spp  <-mean.spp[order(alpha.grad),]

## community responses 
plot(gradient1.season1, mean.spp[,"richness"]) ## explains trends with one outlier
m1 <- lm(richness~gradient1.season1, data=mean.spp)
summary(m1)

plot(gradient1.season1, mean.spp[,"abd"]) ## explains trends with one outlier
m2 <- lm(abd~gradient1.season1, data=mean.spp) ## trend
summary(m2)

plot(gradient2.season1, mean.spp[,"biomass"]) ## explains trends with one outlier
m3 <- lm(biomass~gradient1.season1, data=mean.spp) ## trend
summary(m3)
...
comm2016 <- merge(comm2016, site.vars, by="Site")
...
with(community, points(rda1, display = "sites", col = "black", pch = c(21), bg = colvec[Site]))
orditorp(rda1, display = "species", cex = 0.7, col = "darkred", priority=spp.priority, air=0.5)
text(rda1,  display = "bp", col = "blue", cex = 0.8) 
legend("bottomright", pch=c(21), legend=unique(comm2016$Site), pt.bg=colvec, cex=1)
