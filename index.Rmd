...
## select site
community.site <- subset(community, Site==site.names[1])
...
library(merTools)
## richness with revised confidence band
pred <- cbind(community.arid, predictInterval(m2, community.arid))

p1 <- ggplot(community.arid) + geom_jitter(aes(x=gams, y=Richness, shape=Microsite),color="#181818", alpha=0.4,size=3, width= 0.75) + 
  theme_Publication()+ ylab("Species richness") + xlab("") + annotate("text", x=50,y=6, label="a", size=8) + xlab("Gams index of continentality")+## aesthetics for graph
   geom_ribbon(data= test, aes(x=gams, ymin = CI.low, ymax = CI.high, color=Microsite), size=2, alpha = 0.2, linetype = 0) +  geom_line(data= test, aes(gams, fit, lty = Microsite))

test <- pred %>% group_by(gams, Microsite) %>% summarize(CI.high=mean(exp(upr)),CI.low=mean(exp(lwr)),fit=exp(mean(fit)))


mm<-model.matrix(~gams*Microsite,community.arid)
predFun<-function(.) exp(mm%*%fixef(.)) 
bb<-bootMer(m2,FUN=predFun,nsim=200)
...
p1 <- ggplot(community.arid) + geom_jitter(aes(x=gams, y=Richness), color="#181818", size=3, alpha=0.4, width = 0.75,shape = 1, data=subset(community.arid, Microsite=="shrub"))+geom_jitter(aes(x=gams, y=Richness), color="#181818", size=2,alpha=0.4, data=subset(community.arid, Microsite=="open"),shape = 2, width = 0.75)+ theme_Publication() +  geom_line(data= pred, aes(gams, exp(fit), fill = Microsite)) +
 geom_ribbon(data= pred, aes(x=gams, ymin = lwr, ymax = upr,size=2, alpha = 0.5, linetype = 0)
...
p11 <- ggplot(census.npan, aes(x=gams, y=presence, fill=species, color=species))  + theme_Publication()+  
  ...
  geom_smooth(method="glm", method.args=list(family="binomial"), formula= y~x,aes(x=gams, y=presence),fill="#0daec2", color="#0daec2",  data=subset(census.npan, species=="P. tanacetifolia"& Microsite=="open"), lwd=2, lty=2) +  geom_smooth(method="glm", method.args=list(family="binomial"), formula= y~poly(x,2),aes(x=gams, y=presence), color="#fb6f70", fill="#fb6f7050",  data=subset(census.npan, species=="P. insularis"), lwd=2)+ geom_smooth(method="glm", method.args=list(family="binomial"), formula= y~poly(x,2),aes(x=gams, y=presence),fill="#b7d73850", color="#b7d738",  data=subset(census.npan, species=="S. columbariae"), lwd=2)+ ylab("Probability of occurrence") + xlab("")
...
### Appendix - Microclimate over season
```{r warning=FALSE, message=FALSE}
##

## calculate experimental day
HOBO.season2 <- data.frame(HOBO.season)
HOBO.season2[,"date"] <- as.Date(paste(HOBO.season$Year,HOBO.season$Month,HOBO.season$Day, sep="/"),"%Y/%m/%d")
## season 1
HOBO.season2[HOBO.season2$season=="season.1","exp.day"] <- as.numeric(HOBO.season2[HOBO.season2$season=="season.1","date"])-as.numeric(as.Date(c("2015/11/01"),"%Y/%m/%d"))
## season 2
HOBO.season2[HOBO.season2$season=="season.2","exp.day"] <- as.numeric(HOBO.season2[HOBO.season2$season=="season.2","date"])-as.numeric(as.Date(c("2016/11/01"),"%Y/%m/%d"))
 HOBO.season2[HOBO.season2$Site=="TejonRanch" & HOBO.season2$season=="season.1"  & HOBO.season2$exp.day==151,"exp.day"] <- 14

# ## Conert site names to deserts
# desert <- data.frame(Site=unique(HOBO.season2$Site),desert=c("Mojave","San Joaquin","Mojave","San Joaquin","Mojave","Mojave","San Joaquin"))
# desert.hobo <- merge(HOBO.season2,desert, by=c("Site"))


## Temperature and humidity over years
ggplot(HOBO.season2) + geom_line(aes(x=exp.day, y=Temp, col=Microsite)) + facet_grid(Site~season) + ylab("Temperature (C°)")+theme_Publication() + xlab("Experimental Day")

ggplot(HOBO.season2) + geom_line(aes(x=exp.day, y=RH, col=Microsite)) + facet_grid(Site~season) + ylab("Relative Humidity (%)")+theme_Publication() + xlab("Experimental Day")
...
## season 1
data[data$season=="season.1","exp.day"] <- as.numeric(data[data$season=="season.1","Date"])-as.numeric(as.Date(c("2015/11/01"),"%Y/%m/%d"))
## season 2
data[data$season=="season.2","exp.day"] <- as.numeric(data[data$season=="season.2","Date"])-as.numeric(as.Date(c("2016/11/01"),"%Y/%m/%d"))


ggplot(data) + geom_bar(aes(x=exp.day, y=Precip), stat="identity") + facet_grid(Site~season) + ylab("Precipitation (mm)")+theme_Publication() + xlab("Experimental Day")
