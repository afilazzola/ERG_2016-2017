...
## remove nitrogen because of collinearirty?
vifstep(site.vars, th=10)
...
## season1
season1.mean <- season1 %>% group_by(Gradient,Site) %>% summarise(temp.var=var(avg.temp,na.rm=T),Precip=sum(Precip),min.temp=mean(min.temp, na.rm=T))
...
site.vars <- data.frame(season1.mean[,3:4],nutrients.vars["N"],site.gps["elevation"]) ## drop Phosphorus and other climate variables because of correlations, 
row.names(site.vars) <- weather.vars[,1]
cor(site.vars)
...
## season2
season2.mean <- season2 %>% group_by(Gradient,Site) %>% summarise(temp.var=var(avg.temp,na.rm=T),Precip=sum(Precip, na.rm=T),min.temp=mean(min.temp, na.rm=T))
season2.mean  <- data.frame(season2.mean)
## extract key variables
## dropped RH, and chose min.temp because least correlation with others & cold stress

##combine nutrients and long-term averages
site.vars <- data.frame(season2.mean[,3:4],nutrients.vars["N"],site.gps["elevation"]) ## drop Phosphorus and other climate variables because of correlations, 
row.names(site.vars) <- weather.vars[,1]
cor(site.vars)
...
plot(gradient1.season1, phyto.2016[phyto.2016$Microsite=="open","phyto.biomass"], ylim=c(0,2))
points(gradient1.season1, phyto.2016[phyto.2016$Microsite=="shrub","phyto.biomass"], pch=19)

phyto.2017 <- subset(mean.phyto, Year==2017)

plot(gradient1.season2, phyto.2017[phyto.2017$Microsite=="open","phyto.biomass"], ylim=c(0,2))
...
rda1 <- rda(community.trans, comm2016[,54:58])
...
##
plot(log(rii.precip[,"Precip"]), rii.precip[,"avg"])

m1 <- lm(avg~ log(Precip), data=rii.precip)
